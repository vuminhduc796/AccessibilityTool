...
def sign_apk(apk_name, decompileAPKPath, repackagedAppPath):
    ...
    repackZippedName = apk_name  + "_temp" + ".apk"
    repackAppPathZipped = os.path.join(decompileAPKPath + '/dist', repackZippedName)
    ...
    read, write = os.pipe()
    os.write(write, '123456'.encode(encoding='UTF-8'))
    os.close(write)

    print(keyPath, sign_apk, repackAppPathZipped)
    ...
    if not os.path.exists(ManifestPath):
        ...
        for line in open(ManifestPath,'r').readlines():
            if line.strip().startswith('<activity '):
                ...
                newlines.append(line)
        open(ManifestPath,'w').writelines(newlines)
...
def startRepkg(apk_path, apkname, results_folder, config_folder):
    ...
    decompileAPKPath = decompileAPKPath
    print(decompileAPKPath)
